...
Accepts async map-frame goals (x,y) or "wander" mode and computes velocity 
commands at ~30Hz using the current 2D occupancy costmap.

EXPERIMENT: mppi-costmap-v0

Implementation Phases (from literature review):
  Phase 1: MPPI on 2D costmap (this file) — CUDA or Torch, diff-drive
  Phase 2: iPlanner-IL or SAC-polar (learned MPC)
  Phase 3: ViNT/NoMaD + Doctor (visual policies over MPPI base)

Architecture:
  - Non-blocking mailbox: set_goal() never blocks vision/capture thread
  - Main loop (30Hz): reads latest goal, calls tick() → (fwd, ang)
  - MPPI samples trajectories in costmap window (~2m × 2m ego-space)
  - Rolling subgoals: 0.8-1.0m ahead on path to map-frame goal

Units:
  - World/map frame: x=right, y=up, theta=0 facing +x (CCW), meters/radians
...
class LocalExecutive:
    ...
    def __init__(self,
        ...
        """
        Initialize LocalExecutive with MPPI parameters.
        ...
        print("LocalExecutive: initialized (mppi-costmap-v0, Phase 1 STUB)")
    ...
    def set_goal(self, x: float, y: float) -> None:
        ...
        self._active = True
        print("LocalExecutive: goal set to (%.2f, %.2f) map frame" % (x, y))
...
if __name__ == "__main__":
    print("LocalExecutive unit test (mppi-costmap-v0, Phase 1 stub)")
    
    # Create instance
    le = LocalExecutive(n_samples=128)  # Smaller for unit test
