...
class SafetyGuard:
    ...
    def update(self, obs_map, yaw_delta, fwd_delta, height_cm=None, topdown_near_field=False,
        ...
        checkered_mat: True when checkered mat detected (ego-frame vision, no SLAM).
                      Triggers immediate forward stop; reverse allowed if bwd_clear.
        """
        self._tick += 1
        self._hist.append((yaw_delta, fwd_delta))
        self._topdown_near_field = topdown_near_field
        self._checkered_mat = checkered_mat
        ...
        # - checkered_mat: Vision-based ego-frame detector sees checkered door mat
        #   under/near robot (no SLAM). Triggers immediate forward stop.
        ...
        if topdown_near_field:
            ...
            self._fwd_scale = 0.0
            if self._tick % 10 == 0:
                print("safety: CHECKERED MAT REFLEX — vision detected mat under/near robot, "
                      "fwd=0.0, computing bwd/ang normally")
        else:
            self._near_field_reason = None
