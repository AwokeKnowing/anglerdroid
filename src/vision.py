...
class Vision:
    ...
    def __init__(self, rs1_serial, rs2_serial, rgb1_device_id, headless=True,
        ...
        self._viz_known_premask = np.zeros((FRAME_H, FRAME_W), dtype=np.uint8)
        ...
        # Policy observation double-buffers (copyto; no per-call alloc)
        self._policy_ego_obs = np.zeros((FRAME_H, FRAME_W), dtype=np.uint8)
        self._policy_ego_known = np.zeros((FRAME_H, FRAME_W), dtype=np.uint8)
        self._policy_ego_persistent = np.zeros((FRAME_H, FRAME_W), dtype=np.uint8)
        self._policy_ego_height = np.zeros((FRAME_H, FRAME_W), dtype=np.uint8)
        self._policy_global_proj = np.full((FRAME_H, FRAME_W), 128, dtype=np.uint8)
    ...
    def _capture_loop(self):
        ...
        while self._running:
            ...
            if _gpu_combine_result is not None:
                ...
                # VIS procedural fix: snapshot map BEFORE obs_mask punches out the robot.
                # Otherwise underlay under boxes is empty (std=0) and nothing can show through.
                if getattr(self, '_viz_obs_premask', None) is None or self._viz_obs_premask.shape != self._obs_combined.shape:
                    self._viz_obs_premask = np.empty_like(self._obs_combined)
                np.copyto(self._viz_obs_premask, self._obs_combined)
                if getattr(self, '_viz_known_premask', None) is None or self._viz_known_premask.shape != self._known_combined.shape:
                    self._viz_known_premask = np.empty_like(self._known_combined)
                np.copyto(self._viz_known_premask, self._known_combined)
            ...
            # _viz_obs_premask already snapshotted pre-selfmask in combine
    ...
    def get_rs1_mask_overlay(self):
        """Image-2: intact RS1 ego color + additive G/B boost inside self-mask boxes."""
        import cv2
    ...
    def get_rs1_safety_foot_overlay(self):
        """Safety FOOT padded rect (forward-scan box) on ego-aligned RS1 — 1:1 pixels.

        MASK_*/FOOT_* used by safety.py forward strip — not depth self-mask, not trust FOV.
        """
        import cv2
        if self._rs1 is None or not self._rs1.ok or self._rs1.color is None:
            return None
        ...
        x0, y0, x1, y1 = FOOT_X0, FOOT_Y0, FOOT_X1, FOOT_Y1
        x0, y0 = max(0, min(w, x0)), max(0, min(h, y0))
        x1, y1 = max(0, min(w, x1)), max(0, min(h, y1))
        if x1 > x0 and y1 > y0:
            m = np.zeros((h, w), dtype=bool)
            m[y0:y1, x0:x1] = True
            overlay[m, 0] = np.clip(rgb[m, 0].astype(np.int16) + 80, 0, 255).astype(np.uint8)
            overlay[m, 1] = np.clip(rgb[m, 1].astype(np.int16) + 40, 0, 255).astype(np.uint8)
            overlay[m, 3] = 110
            cv2.rectangle(overlay, (x0, y0), (x1 - 1, y1 - 1), (255, 160, 0, 220), 2)
            fx = min(w - 1, max(0, x1 - 1))
            overlay[y0:y1, fx, :] = (0, 255, 255, 220)
        return overlay


    def get_safety_foot_overlay(self):
        """Ego 1cm/px: safety FOOT padded rect (orange) vs bot hull in robot_foot_overlay."""
        import cv2
    ...
    def get_policy_observation(self):
        """Labeled ego heightmap for neural/MPPI policy (Tesla FSD-style).

        Returns a dict of uint8 layers + metadata, or None if topdown is not OK.
        Arrays are references to preallocated buffers — valid until the next call.
        See POLICY_HEIGHTMAP_API.md.
        """
        # Snapshot ego layers under lock (copyto into prealloc; no fresh alloc).
        with self._lock:
            if not getattr(self, '_topdown_ok', False):
                return None
            np.copyto(self._policy_ego_obs, self._obs_combined)
            np.copyto(self._policy_ego_known, self._known_combined)
            np.copyto(self._policy_ego_persistent, self._persistent_obs)
            np.copyto(self._policy_ego_height, self._persistent_height)
            pose_x = float(self._pose.x)
            pose_y = float(self._pose.y)
            pose_theta = float(self._pose.theta)
            topdown_ok = bool(self._topdown_ok)
            slam_locked = bool(self._slam_locked)
        ...
        return {
            ...
            'ego_known': self._policy_ego_known,
            'ego_persistent': self._policy_ego_persistent,
            'ego_height': self._policy_ego_height,
            'global_projected_conf': self._policy_global_proj,
            'metadata': {
                'ego_h': FRAME_H,
                'ego_w': FRAME_W,
    ...
    def get_robot_footprint_overlay(self):
        ...
        if n % 15 == 1:
            try:
                ...
                _cv.imwrite('/home/jetbot/.kevin/overlays/live_under.png',
                            _cv.resize(_cv.cvtColor(rgb, _cv.COLOR_RGB2BGR), (w*3, h*3), interpolation=_cv.INTER_NEAREST))
            except Exception:
                pass
        return overlay
