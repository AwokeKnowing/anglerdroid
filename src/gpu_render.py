...
# ── Shaders for GPU topdown depth (RS1) ──────────────────────────
...
class GPURenderer:
    ...
    # ── Depth-topdown configuration (call once before use) ────────

    def configure_depth_topdown(self, px_size, floor_clip, out_h, out_w):
        """Configure GPU topdown depth (RS1 orthographic projection).
        
        Args:
            px_size: Pixel size in metres (EGO_PX_SIZE, typically 0.01)
    ...
    def _init_gl(self):
        ...
        # Blurred obstacle mask (variable-radius box blur of binarized conf)
    ...
    # ── Depth-topdown GL init ─────────────────────────────────────

    def _init_depth_topdown_gl(self):
        """Initialize GPU topdown depth (RS1 orthographic scatter)."""
        ctx = self._ctx
        ...
        self._dt_n = 0
        print("gpu_render: depth_topdown ready %dx%d (orthographic ego scatter)"
              % (ow, oh))
    ...
    # ── GPU depth-topdown pipeline ────────────────────────────────

    def depth_topdown_gpu(self, verts):
        """Process RS1 topdown depth on GPU: orthographic scatter → (obs, known).
        
        Args:
            verts: Nx3 point cloud from RS1 (X, Y, Z in metres)
