...
def test_using_encoder_feedback_defined_before_use():
    ...
    for i, line in enumerate(lines):
        if 'using_encoder_feedback = False' in line:
            # Check context - should be near encoder health logic
            context = ' '.join(lines[max(0, i-2):min(len(lines), i+2)])
            if 'encoder' in context.lower():
...
def test_encoder_feedback_logic_check():
    ...
    # Check that the logic checks encoder_ok and age_s threshold
    ...
    # The hotfix uses either .get() or direct access, both are acceptable
    has_health_check = ("health.get('encoder_ok')" in content or 
                        "health['encoder_ok']" in content)
    assert has_health_check, "Missing encoder_ok health check"
    
    print("✓ PASS: encoder feedback logic checks encoder health")
    return True
