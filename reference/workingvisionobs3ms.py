...
profile = pipeline_td.get_active_profile()
...
def pointcloud(out, verts, texcoords, color, painter=False):
    ...
    depth_values = (255-((verts[m][:, 2]+.25)*400).astype(np.uint8))-48
    ...
    # perform uv-mapping
    out[i[m], j[m]] = depth_values#color[u[m], v[m]]
    ...
    #cv2.adaptiveThreshold(out, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 5, 0,dst=out)
    #cv2.GaussianBlur(out,(3,3),0,dst=out)
    ret3,th3 = cv2.threshold(out,0,255,cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU,dst=out)
...
while True:
    ...
    if not state.paused:
        ...
        depth_frame = frames_td.get_depth_frame()
        color_frame = frames_td.get_color_frame()
        ...
        if False:
            ...
            # Rotate each vertex around the origin by 45 degrees on the y-axis
            verts = np.dot(verts, rotation_matrix)
            
            if False:
                # Extract z-values from point_cloud
                z_values = verts[:, 2]
                z_values /= state.color_scale

                # Replace both elements of texcoords with z-values
                texcoords[:, 0] = z_values
                texcoords[:, 1] = z_values
    ...
    if True or not state.scale or out.shape[:2] == (h, w):
        pointcloud(out, verts, texcoords, color_source)
        #cout =pointcloud(out, verts, texcoords, color_source)
    else:
