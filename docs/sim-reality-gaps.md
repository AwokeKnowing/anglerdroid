...
- 2026-09-06 ~12:34 PT: Soft prefer A/B enjoy GIFs (hallway + couch_pinch, seed=42). CLI `--soft-cost`/`--no-soft-cost` + `--seed` on `sim/run.py`; `MppiSimPolicy(use_soft_cost=True)` default. Soft ON vs OFF both **0 collisions** (exit 0); soft ON tends CCW corridor bias (hallway θ≈−34°, couch θ≈−29°) vs soft OFF CW (hallway θ≈+34°, couch θ≈+47°). GIFs under `artifacts/sim/*_mppi_soft_{on,off}.gif`. Live soft_scales still default OFF; live drive still stopped.

- 2026-09-06 ~13:06 PT: Corridor-centering metric (`mean_lat_clear_m` / `min_lat_clear_m` in `sim/run.py`) + hallway soft ON vs OFF seed sweep (7/11/42/99/123): soft avg **0.263** m vs hard **0.259** m, 0 collisions. Fidelity=True + soft_cost OFF hard-mask residual clean on couch_pinch/hallway/doorway/cul_de_sac @400. Transfer **10/10**, sim 24/24, mask 12/12, dual 8/8, faces 14/14. Live soft_scales still default OFF; live drive still stopped.
